...
def train_robust(model, train_loader, val_loader, epsilon, start_epoch, num_epochs, optimizer, criterion, device, train_losses, train_metrics_clean, train_metrics_adv, val_metrics):
    ...
    }, f'models_10/fgsm_resnet50/resnet50_fgsm_epoch_{epoch+1}_LR_{LR}_batchsize_{BATCH_SIZE}_WD_{WD}_EPS_{epsilon}_fine_tuned.pt')
    print("Model saved in models_10/fgsm_resnet50")
   ...
   # saving metrics history
    ...
    save_history_json(history,f"history_10/history_fgsm/history_fgsm_epoch_{epoch+1}_LR_{LR}_batchsize_{BATCH_SIZE}_WD_{WD}_EPS_{epsilon}_fine_tuned.json")
...
if __name__ == "__main__":
    ...
    #train_small = balanced_subset(train_dataset, n_per_class=32)
...
#
    ...
    plot_metric(train_metrics_clean.accuracy_list, val_metrics.accuracy_list, NUM_EPOCHS, "Accuracy", 
                f"metrics_images_fgsm/Train_accuracy_plot_numepochs_{NUM_EPOCHS}_LR_{LR}_batchsize{BATCH_SIZE}_WD_{WD}_EPS_{eps}.png")
    #plot f1 score
    plot_metric(train_metrics_clean.f1_list,  val_metrics.f1_list, NUM_EPOCHS, "F1_score", 
                f"metrics_images_fgsm/Train_F1_plot_numepochs_{NUM_EPOCHS}_LR_{LR}_batchsize{BATCH_SIZE}_WD_{WD}_EPS_{eps}.png")
    #plot precision
    plot_metric(train_metrics_clean.precision_list,  val_metrics.precision_list, NUM_EPOCHS, "Precision",
                f"metrics_images_fgsm/Train_precision_plot_numepochs_{NUM_EPOCHS}_LR_{LR}_batchsize{BATCH_SIZE}_WD_{WD}_EPS_{eps}.png")
    #plot recall
    plot_metric(train_metrics_clean.recall_list,  val_metrics.recall_list, NUM_EPOCHS, "Recall", 
                f"metrics_images_fgsm/Train_recall_plot_numepochs_{NUM_EPOCHS}_LR_{LR}_batchsize{BATCH_SIZE}_WD_{WD}_EPS_{eps}.png")
    #plot AUC
    plot_roc(val_metrics.fpr, val_metrics.tpr, val_metrics.auc_list[NUM_EPOCHS-1], NUM_EPOCHS, 
             f"metrics_images_fgsm/Train_ROC_plot_numepochs_{NUM_EPOCHS}_LR_{LR}_batchsize{BATCH_SIZE}_WD_{WD}_EPS_{eps}.png")
