...
class FFDataset(Dataset):
    def __init__(self, root_dir, split, transform=None, manipulation="Deepfakes"):
        ...
        real_path = os.path.join(
            root_dir, "data", "original_sequences", "youtube", "c23", "frames"
        )
        for vid in video_ids:
            ...
            if os.path.exists(frame_dir1):
                for frame in os.listdir(frame_dir1):
                    self.samples.append((os.path.join(frame_dir1, frame), 0, f"r.c23.{vid1}.{frame.split('.')[0]}"))

            if os.path.exists(frame_dir2):
                for frame in os.listdir(frame_dir2):
                    self.samples.append((os.path.join(frame_dir2, frame), 0, f"r.c23.{vid2}.{frame.split('.')[0]}"))

        # FAKE
        fake_path = os.path.join(
            root_dir, "data", "manipulated_sequences", manipulation, "c23", "frames"
        )
        for vid in video_ids:
            vid1 = vid[0]
            vid2 = vid[1]
            ...
            if os.path.exists(frame_dir1):
                for frame in os.listdir(frame_dir1):
                    self.samples.append((os.path.join(frame_dir1, frame), 1, f"f.c23.{vid1}_{vid2}.{frame.split('.')[0]}"))

            if os.path.exists(frame_dir2):
                for frame in os.listdir(frame_dir2):
                    self.samples.append((os.path.join(frame_dir2, frame), 1, f"f.c23.{vid2}_{vid1}.{frame.split('.')[0]}"))
