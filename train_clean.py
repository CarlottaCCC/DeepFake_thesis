...
def train_clean_f(model, train_loader, val_loader, start_epoch, num_epochs, optimizer, criterion, device, train_losses, train_metrics, val_metrics, seed=42):
    history = {}
    ...
    model_name = f'{MODELS_DIR}/resnet18/resnet18_clean_epoch_{epoch+1}_lr_{lr}_wd_{wd}_aug.pt'
    ...
    save_history_json(history,f"history/resnet18/history_clean_resnet18_epoch_{epoch+1}_lr_{lr}_wd_{wd}_aug.json")
...
if __name__ == "__main__":
    ...
    # MAIN MODEL: Modello ResNet50 with pretrained (with no pretrained weights=False)
    #model = resnet50(weights=ResNet50_Weights.IMAGENET1K_V1)

    # SURROGATE MODEL FOR TRANSFERABILITY ROBUSTNESS
...
#
    ...
    train_metrics, val_metrics, train_losses, model_name = train_clean_f(
        model=model, 
