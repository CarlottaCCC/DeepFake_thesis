...
def train_las_pgd_at(model, train_loader, val_loader, num_epochs, optimizer, LRscheduler, criterion, device, train_detector_losses, train_scheduler_losses, train_metrics_clean, train_metrics_adv, val_metrics_clean, val_metrics_adv, 
                  eps=8/255, alpha=2/255, steps=20, lr=1e-3,
                  # --- ADES-specific ---
                  ades_scheduler=None, ades_optimizer=None, ades_eps_min=2/255,
                  ades_eps_lambda=6/255, ades_mc_passes=3, lambda_corr = 0.1, seed=0, save_model=True):
    ...
    print(f"Training PGD-AT | mode=LAS-ADES | eps={eps:.4f} alpha={alpha:.4f} steps={steps}")
 
    for epoch in range(num_epochs):
        epoch_start_time = time.perf_counter()
        model.train()
        train_detector_loss, train_scheduler_loss, train_correct_clean, train_correct_adv, n_seen = 0.0, 0.0, 0, 0, 0
        epoch_eps = []
        ...
        for batch_idx, batch in enumerate(loop):
            ...
            # DETECTOR UPDATE #######################
            ## I freeze the scheduler
            #for p in ades_scheduler.parameters():
            ...
            imgs_adv = normalize(imgs_adv_raw) # detach() or not detach()???
            ...
            #logits_adv = model(imgs_adv)
            #loss_adv_scheduler = criterion(logits_adv, y)
            
            per_sample_adv_loss = F.cross_entropy(logits_adv, y, reduction="none")
            difficulty = per_sample_adv_loss.detach()
            difficulty = (difficulty - difficulty.mean()) / (difficulty.std() + 1e-8)
            sigma_norm = (sigma - sigma.mean()) / (sigma.std() + 1e-8)

            correlation = F.cosine_similarity(
                sigma_norm.unsqueeze(0),
                difficulty.unsqueeze(0),
                dim=1
            ).mean()

            corr_history.append(correlation.item())

            scheduler_loss = (
                - lambda_corr * correlation
            )

            if batch_idx % 50 == 0:
                print(
                    f"eps mean={eps_x.mean():.5f}, "
                    f"std={eps_x.std():.5f}, "
                    f"corr={correlation.item():.3f},"
                    f"adv_loss={loss_adv.item()},"
                    f"scheduler_loss={scheduler_loss.item()}"
                )
...
#
            #print("scheduler grad:", total_norm)
            ...
            train_detector_loss += loss.item() * imgs.size(0)
            train_scheduler_loss += scheduler_loss.item() * imgs.size(0)
        ...
        epoch_detector_loss = train_detector_loss / max(n_seen, 1)
        train_detector_losses.append(epoch_detector_loss)

        epoch_scheduler_loss = train_scheduler_loss / max(n_seen, 1)
        train_scheduler_losses.append(epoch_detector_loss)
        ...
        print(f"Epoch {epoch+1} [LAS_ADES] | train_detector_loss={epoch_detector_loss:.4f} val_loss={epoch_val_loss:.4f} train_scheduler_loss={epoch_scheduler_loss:.4f} "
    ...
    print(f"[TIMING] Total: {total_time:.2f}s ({total_time/60:.2f} min) over {num_epochs} epochs, "
          f"mode=LAS-ADES, avg {sum(epoch_total_times)/max(len(epoch_total_times),1):.2f}s/epoch")

    mode = f"las_ades_{lambda_corr}"

    
    save_path = f'{MODELS_DIR}/pgdat_ades_difat/resnet50_pgdat_{mode}_lr_{lr}_seed_{seed}_epochs_{num_epochs}_{loss_type}.pt'
    history_path = f"history/history_pgdat_ades_difat/history_pgdat_{mode}_lr_{lr}_seed_{seed}_epochs_{num_epochs}_{loss_type}.json"
 
    if save_model:
        torch.save({
            "mode": mode, "model_state_dict": model.state_dict(),
            "optimizer_state_dict": optimizer.state_dict(),
            "train_detector_losses": train_detector_losses,
            "train_scheduler_losses": train_scheduler_losses,
    ...
    return model, train_metrics_clean, train_metrics_adv, val_metrics_clean, val_metrics_adv, train_detector_losses, train_scheduler_losses, epoch_total_times
...
#
    ...
    start_epoch = 0
    train_detector_losses = []
    train_scheduler_losses = []
    num_epochs = 20
    alpha_adv = 0.5
    epsilon = 8/255
    ...
    lambda_ades = 0.001
    ##############################
    print(f"Starting training PGD-AT with mode: LAS-ADES-PGD-AT")
    
    trained_model, train_metrics_clean, train_metrics_adv, val_metrics_clean, val_metrics_adv, train_detector_losses, train_scheduler_losses, epoch_total_times = train_las_pgd_at(
        model=model,
    ...
    out_dir = f'pgd_las_ades'
    os.makedirs(out_dir, exist_ok=True)
    mode = f"las_ades_lambda_corr_{lambda_corr}"
    
    with open(f'{out_dir}/grid_search_pgd_{mode}_lr_{lr}_{num_epochs}_alpha_{alpha_adv}.json', 'w') as f:
