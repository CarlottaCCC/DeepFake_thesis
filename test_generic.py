...
def test_attack(model, test_loader, attack_type, epsilon, library, model_name, device):
    ...
    for batch in pbar:
        ...
        if library == 'foolbox' and attack_type != 'genattack':
            ...
            target_labels = 1 - labels
            imgs_adv = nes_attack(model, imgs, target_labels)
            ...
            imgs_adv = attack.generate(imgs.cpu().numpy(), labels.cpu().numpy())
            imgs_adv = torch.from_numpy(imgs_adv).float().to(device)
        ...
        with torch.no_grad():
            ...
            print("clean preds:", preds.cpu().numpy())
    ...
    save_history_json(history,f"test_results_{attack_type}/{model_name}/results_{attack_type}_eps_{epsilon}_LR_{LR}_batchsize_{BATCH_SIZE}_WD_{WD}_num_samples_{total_samples}.json")
